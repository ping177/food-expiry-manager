...
- 将“外部兜底图片链接（可选）”简化为“外部图片链接（可选）”，并统一分类与图片链接控件的顶部和高度对齐。
...
### v0.2.12-D 商品容量 / 规格（结构化修正）

- 远程已执行的 legacy `products.size` text migration 保留；新增 `size_value numeric` 与 `size_unit text` 兼容 migration，不删除、不回填 legacy 字段。
- 新增和编辑商品改为数值输入与 `g` / `kg` / `ml` / `L` 下拉；空容量保存为两个 `null`，展示时组合为如 `170g`。
- 新增与编辑页的商品次级字段改为两行两列：品牌与容量规格同列、分类与图片链接同列；容量数值和单位保持同一行，默认单位为 `g`，空数值仍保存为两个 `null`。
- 无 barcode 商品复用键调整为名称 + 品牌 + 容量数值 + 容量单位；数值或单位不同均保留为独立 product。
- 条码流程只解析 API 明确提供的容量字段，不从商品名称猜测；首页卡片、详情与新增库存摘要仅在容量完整时展示。
- `npm test` 通过 18 个测试文件 / 167 个测试，`npm run build` 成功，`git diff --check` 通过。
