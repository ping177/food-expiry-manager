...
- 新增固定双 Tab 导航，仅包含“库存”和“我的”；新增商品、库存详情和编辑任务流不显示 Tab。
- 库存页保留搜索、到期时间筛选、分类筛选、商品列表和添加商品入口；添加入口改为内容区按钮，避免遮挡底部导航。
- “我的”页增加提醒设置、数据导出、偏好设置的“开发中”禁用占位，不创建空页面。
- 未修改 Supabase Auth flow、schema、RLS、环境变量、库存 API 或数据模型。
- 新增导航组件测试；`npm test` 通过 15 个测试文件 / 131 个测试，`npm run build` 成功，`git diff --check` 通过。
