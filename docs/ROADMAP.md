...
- 状态：已完成本地实现；自动化测试和生产构建通过，PWA 真机视觉验收待完成
- 顶层导航仅包含带内置 SVG 图标的“库存”和“我的”两个 Tab，居中的无文字 `+` 作为新增商品操作而非第三个 Tab；默认灰色，选中 Tab 使用现有绿色
- 库存页保留列表、搜索和筛选；移除重复内容区添加按钮及冗余 Header 文案
- 底栏采用轻量页面融合样式，并使用 iPhone PWA 安全区与对应内容留白
- “我的”页承载账号信息和退出登录，提醒设置、数据导出、偏好设置仅作“开发中”占位
- 不修改 Supabase Auth、schema、RLS、环境变量、库存逻辑或数据模型
